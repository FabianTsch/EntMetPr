...
class Bilder:
    ...
    def ausrichten(self):
        ...
        points_dst = np.array(
            [[self.x, 0], [0, 0], [0, self.y], [self.x, self.y]])
    ...
    def getOrientation(self, mu):
        ...
        if self.plot == True:
            ...
            color = (0, 0, 0)
            thickness = 1
            cv2.putText(self.im_dst, str(alpha), (x, y),
                        font, fontScale, color, thickness)
            image = cv2.line(
                self.im_dst, center, (x+int(ev[0, 0]*s), y+int(ev[1, 0]*s)), (0, 255, 0), 2)
            self.image = cv2.line(
                self.im_dst, center, (x+int(ev[0, 1]*s*3), y+int(ev[1, 1]*s*3)), (0, 0, 0), 2)
